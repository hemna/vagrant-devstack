...
if is_service_enabled nova; then
    ...
    # Add any other public keys in authorized_keys
    idx=0
    grep -v -e '^#' $HOME/.ssh/authorized_keys -e 'vagrant insecure' |
    while read keytype key comment ; do
        if [[ "$keytype" = "ssh-rsa" ]] ; then
            ((idx++))
            file=$(tempfile)
            echo $keytype $key "$comment" > $file
            nova keypair-add --pub_key=$file personal-key-$idx
            #rm $file
        fi
    done
